...
def create_excel_tab(notebook, state, callbacks, build_recipient_tree, enable_treeview_sort):
    ...
    ttk.Button(
        actions,
        text="Sammelmail erstellen",
        command=lambda: _force_excel_mode(state["excel_mode_var"], "collective", callbacks["prompt_excel_compose"]),
...
def _force_excel_mode(mode_var, mode_value, prompt_fn):
    """Force the Excel mode before opening the compose dialog."""
    try:
