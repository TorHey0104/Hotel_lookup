...
def render_with_signature(body_text: str, signature_entry: dict, body_is_html: bool = False, forward_html: str = "", forward_is_html: bool = False):
    ...
    user_block = wrap_block(body_text if body_is_html else to_html(body_text))

    # Signature block
    sig_block = ""
    if sig_html:
        sig_block = wrap_block(sig_html if looks_like_html(sig_html) else to_html(sig_html))
    elif sig_txt:
        sig_block = wrap_block(to_html(sig_txt))
    ...
    if forward_html:
        if forward_is_html or looks_like_html(forward_html):
            ...
            forward_block = wrap_block(to_html(forward_html))
